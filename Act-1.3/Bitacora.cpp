...
int Bitacora::getSize() { return listaRegistros.size(); }
...
void Bitacora::sortMerge() { mergeSort(0, listaRegistros.size() - 1); }
...
void Bitacora::busqueda() {
  ...
  if (indInicio == -1 || indFin == -1) {
    ...
    resultado(indInicio, indFin,Cr);
  }
}

void Bitacora::resultado(int i, int f,int Cr) {
  std::ofstream resultado_busqueda("resultado_busqueda.txt");
  while (i <= f) {
    std::cout << listaRegistros[i].getAll() << std::endl;
    Registro r = listaRegistros[i];
    resultado_busqueda << r.to_string() << std::endl;
    i++;
  }
  std::cout << "Resultado: " << Cr << " Registros" << std::endl;
}

void Bitacora::escribir() {
  std::ofstream file2("bitacora_ordenada.txt");
  for (int i = 0; i < listaRegistros.size(); i++) {
    Registro g = listaRegistros[i];
    file2 << g.to_string() << std::endl;
  }
}
